...
// አዲስ፡ አይተም ለመቀነስ/ለማጥፋት (Delete Item)
function deleteItem(index) {
    boqList.splice(index, 1); // ከዝርዝሩ ላይ ያስወግደዋል
    updateTable(); // ሰንጠረዡን በድጋሚ ያድሳል
    if(document.getElementById('resultBox').style.display === 'block') {
        calculateFinalTender(); // ዋጋው አስልቶ ከነበረ ዋጋውንም በራስ-ሰር ያድሳል
    }
}
...
// 2. ፎቶ አንስቶ በራሱ ፅሁፍና ቁጥር የመለየት ስራ (AI Scan)
function scanImage() {
...
// 3. የተሻሻለ የጨረታ ዋጋ ስሌት (ከነፐርሰንት ማስተካከያው)
function calculateFinalTender() {
    ...
    // አዲሱ ንዑስ ድምር (Subtotal) ከማስተካከያው በኋላ
    const subTotal = baseDirectCost + adjustmentAmount; 
    const vat = subTotal * 0.15; // 15% VAT
    const grandTotal = subTotal + vat; // Total with VAT
